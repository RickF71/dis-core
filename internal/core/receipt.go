...
func ListReceipts(db *sql.DB) {
	rows, err := db.Query(`SELECT id, identity_id, action, by_domain, scope, nonce, timestamp, policy_checksum, signature, created_at
	...
	for rows.Next() {
		...
		if len(sig) > 16 { sig = sig[:16] + "..." }
		if len(polsum) > 16 { polsum = polsum[:16] + "..." }
