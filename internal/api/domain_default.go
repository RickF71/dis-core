...
func (s *Server) handleGetDefaultDomain(w http.ResponseWriter, r *http.Request) {
	...
	row := s.DB().QueryRow(ctx, `SELECT id, parent_id, data, created_at, updated_at FROM domains WHERE name = 'domain.void' LIMIT 1`)

	var d Domain
	if err := row.Scan(&d.ID, &d.ParentID, &d.Data, &d.CreatedAt, &d.UpdatedAt); err != nil {
		// fallback: try domain.terra if void missing
		row2 := s.DB().QueryRow(ctx, `SELECT id, parent_id, data, created_at, updated_at FROM domains WHERE name = 'domain.terra' LIMIT 1`)
