...
func (s *Server) handleDomainInfo(w http.ResponseWriter, r *http.Request) {
	...
	if code == "" {
		JSON(w, http.StatusBadRequest, map[string]string{"error": "missing 'code' query parameter"})
		return
	}
	...
	if err != nil {
		...
		if strings.HasPrefix(msg, "400 ") {
			JSON(w, http.StatusBadRequest, map[string]string{"error": strings.TrimPrefix(msg, "400 ")})
			return
		}
		if strings.HasPrefix(msg, "404 ") {
			JSON(w, http.StatusNotFound, map[string]string{"error": strings.TrimPrefix(msg, "404 ")})
			return
		}
		JSON(w, http.StatusInternalServerError, map[string]string{"error": msg})
		return
	}
