...
func (s SharedCoreTriad) Validate() error {
	if err := requireNonEmpty("type", s.Type); err != nil {
		return err
	}
	if err := requireNonEmpty("version", s.Version); err != nil {
		return err
	}
	if err := requireNonEmpty("domain", s.Domain); err != nil {
		return err
	}
	if err := requireNonEmpty("parent_domain", s.ParentDomain); err != nil {
		return err
	}
	if err := requireNonEmpty("authorization_domain", s.AuthorizationDom); err != nil {
		return err
	}
	if err := requireNonEmpty("child_domain", s.ChildDomain); err != nil {
		return err
	}
	if err := requireNonEmpty("shared_core_domain", s.SharedCoreDomain); err != nil {
		return err
	}
	if s.Threshold < 3 {
		return fmt.Errorf("threshold must be >= 3 for a stable triad")
	}
	if err := requireNonEmpty("created_at", s.CreatedAt); err != nil {
		return err
	}
	if err := ISO8601(s.CreatedAt); err != nil {
		return err
	}
	if s.Status != "" {
		if err := oneOf("status", s.Status, "forming", "active", "severed"); err != nil {
			return err
		}
	}
	return nil
